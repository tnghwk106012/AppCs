...
class _Particle {
  final double baseX, baseY, baseRadius, amplitude, speed, blur, phase, pulseSpeed, pulseStrength;
  final int colorIdx;
  _Particle({
    required this.baseX,
    required this.baseY,
  ...
  factory _Particle.random(Random random, int colorIdx) {
    return _Particle(
      baseX: 0.5 + (random.nextDouble() - 0.5) * 0.8,
      baseY: 0.5 + (random.nextDouble() - 0.5) * 0.8,
      baseRadius: 22 + random.nextDouble() * 38,
      amplitude: 24 + random.nextDouble() * 48,
      speed: 0.5 + random.nextDouble() * 1.7,
      blur: 14 + random.nextDouble() * 28,
      phase: random.nextDouble() * 2 * pi,
      pulseSpeed: 0.7 + random.nextDouble() * 1.5,
      pulseStrength: 0.08 + random.nextDouble() * 0.22,
...
class _ParticlesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final p in particles) {
      ...
      final radius = p.baseRadius * (0.7 + 0.5 * appear) * pulse;
      final opacity = (0.18 * appear + 0.08) * (0.7 + 0.3 * pulse);
      final paint = Paint()
        ...
        ..maskFilter = MaskFilter.blur(BlurStyle.normal, p.blur * (0.7 + 0.5 * appear) * (0.8 + 0.4 * pulse))
